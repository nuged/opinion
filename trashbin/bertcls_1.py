...
tokenizer = AutoTokenizer.from_pretrained("DeepPavlov/rubert-base-cased", do_lower_case=False)
...
def CV(data, labels, n_epochs, lr):
    ...
    for metric in results:
        results[metric] /= 5
        print(f'{metric} = {results[metric]:4.2f}')
...
if __name__ == "__main__":
    ...
    print(f"it was {len(data)} examples")
    data = remove_duplicates(data)
    print(f"not it is only {len(data)} examples.")
    p.close()
